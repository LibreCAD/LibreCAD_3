...
bool Math::linearSolver(const std::vector<std::vector<double> >& mt, std::vector<double>& sn){
    ...
    for(int i=0;i<mSize;i++){
        ...
        if(cmax<TOLERANCE2) return false; //singular matrix
...
CoordinateSolutions Math::simultaneousQuadraticSolverMixed(const std::vector<std::vector<double> >& m)
{
    ...
    if(p1->size()==3) {
            ...
            ce[0][2]=-ce[0][2];
            ce[1][2]=-ce[1][2];
            if( Math::linearSolver(ce,sn)) ret.push_back(geo::Coordinate(sn[0],sn[1]));
            return ret;
    }
    ...
    if(roots.size()==0)  return CoordinateSolutions();

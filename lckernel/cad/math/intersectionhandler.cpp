...
//    }
    ...
    if(bb2.height() <= BBHEURISTIC2 || bb2.width() <= BBHEURISTIC2) {
        return;
    }


/*    if (!((cmax.x() < bmin.x()) || (cmin.x() > bmax.x()) || (cmax.y() < bmin.y()) || (cmin.y() > bmax.y()))) {
        auto b2split = B2->splitHalf();
        BezCir(C, c_area, b2split[0], ret);
        BezCir(C, c_area, b2split[1], ret);
    } else */

//    if(cmin.x() < bmin.x() && cmin.y() < bmin.y() && cmax.x() > bmax.x() && cmax.y() > bmax.y()) {

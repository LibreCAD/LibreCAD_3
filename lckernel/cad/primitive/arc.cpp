...
shared_ptr<const lc::CADEntity> Arc::move(const geo::Coordinate& offset) const {
    Arc *newarc = new lc::Arc(this->center() + offset, this->radius(), this->startAngle(), this->endAngle());
        newarc->setID(this->id());
    shared_ptr<const lc::Arc> newArc = shared_ptr<const lc::Arc>(newarc);
    return newArc;
}

shared_ptr<const lc::CADEntity> Arc::copy(const geo::Coordinate& offset) const {
    Arc *newarc = new lc::Arc(this->center() + offset, this->radius(), this->startAngle(), this->endAngle());
    shared_ptr<const lc::Arc> newArc = shared_ptr<const lc::Arc>(newarc);
    return newArc;
}

shared_ptr<const lc::CADEntity> Arc::rotate(const geo::Coordinate &rotation_center, const double& rotation_angle, const bool with_same_id) const {
    Arc *newarc = new lc::Arc(this->center().rotate(rotation_center, rotation_angle),

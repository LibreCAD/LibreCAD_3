...
shared_ptr<const lc::CADEntity> Spline::move(const geo::Coordinate& offset) const {
...
shared_ptr<const lc::CADEntity> Spline::copy(const geo::Coordinate& offset) const {
    QList<geo::Coordinate> control_pts;
    for (int i = 0; i < this->control_points().size(); ++i) {
        control_pts.append((this->control_points().at(i)) + offset);
    }
...
shared_ptr<const lc::CADEntity> Spline::rotate(const geo::Coordinate &rotation_center, const double& rotation_angle, const bool with_same_id) const {
    QList<geo::Coordinate> control_pts;
    for (int i = 0; i < this->control_points().size(); ++i) {

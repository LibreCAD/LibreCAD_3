...
shared_ptr<const lc::CADEntity> Line::move(const geo::Coordinate& offset) const {
    Line *newline = new lc::Line(this->start() + offset, this->end() + offset);
    newline->setID(this->id());
    shared_ptr<const lc::Line> newLine = shared_ptr<const lc::Line>(newline);
    return newLine;
}

shared_ptr<const lc::CADEntity> Line::copy(const geo::Coordinate& offset) const {
    Line *newline = new lc::Line(this->start() + offset, this->end() + offset);
    shared_ptr<const lc::Line> newLine = shared_ptr<const lc::Line>(newline);
    return newLine;
}

shared_ptr<const lc::CADEntity> Line::rotate(const geo::Coordinate &rotation_center, const double& rotation_angle, const bool with_same_id) const {
    Line *newline = new lc::Line(this->start().rotate(rotation_center, rotation_angle),

...
void UndoManagerImpl::on_CommitProcessEvent(const lc::CommitProcessEvent& event) {
    ...
    std::tr1::shared_ptr<lc::Undoable>  undoable = std::tr1::dynamic_pointer_cast<Undoable>(event.operation());
    ...
    if (undoable != NULL) {
        ...
        while (canRedo()) {
            std::tr1::shared_ptr<lc::Undoable>  undoable = _reDoables.pop();
...
void UndoManagerImpl::undo() {
    if (canUndo()) {
        std::tr1::shared_ptr<lc::Undoable>  undoable = _unDoables.last();
        _unDoables.pop_back();
        undoable->undo();

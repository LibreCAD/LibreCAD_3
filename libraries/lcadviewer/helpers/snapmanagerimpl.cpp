...
SnapManagerImpl::SnapManagerImpl(LCADViewer* graphicsView, boost::shared_ptr<lc::SelectionManager> selectionmanager, boost::shared_ptr<const lc::Snapable> grid, double distanceToSnap)  : _selectionmanager(selectionmanager), _grid(grid), _distanceToSnap(distanceToSnap) {
...
void SnapManagerImpl::on_mouseMoveEvent(const MouseMoveEvent& event) {
    ...
    if (_entities.count() > 1) {
        ...
        for (int a = 0; a < _entities.count(); a++) {
            for (int b = a + 1; b < _entities.count(); b++) {
                boost::shared_ptr<const lc::CADEntity> i1 = _entities.at(a).entity();
                boost::shared_ptr<const lc::CADEntity> i2 = _entities.at(b).entity();
    ...
    if (_entities.count() > 0) {
        ...
        qSort(_entities.begin(), _entities.end(), lc::EntityDistance::sortAscending);
        const boost::shared_ptr<const lc::Snapable> captr = boost::dynamic_pointer_cast<const lc::Snapable>(_entities.at(0).entity());

...
void GradientBackground::draw(event::DrawEvent const & event) const {
    ...
    qDebug("Gradient Line=%f , %f --- %f , %f",0.0,  updateRect.minP().y(),  0.0, updateRect.maxP().y());
    ...
    painter.set_pattern_source(patId);
     qDebug("Rectangle call=%f , %f --- %f , %f",updateRect.minP().x(), updateRect.minP().y(), updateRect.width(), updateRect.height());
